...
QUERY = """
    SELECT
        airline,
        origin            AS departure_airport_id,
        dest              AS arrival_airport_id,
        crs_dep_time,
        crs_arr_time,
        distance,
        flight_date,
...
def build():
    ...
    df["dep_airport_code"] = df["departure_airport_id"].map(lambda v: encode_category(v, mappings["departure_airport_id"]))
    df["arr_airport_code"] = df["arrival_airport_id"].map(lambda v: encode_category(v, mappings["arrival_airport_id"]))
    ...
    # Sélection finale
    # ------------------------------------------------------------------
    ID_COLS = ["airline", "departure_airport_id", "arrival_airport_id"]
