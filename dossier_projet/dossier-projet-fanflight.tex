...
Docker Compose orchestre l'ensemble des services : base de données, API, frontend, serveur MLflow et conteneur ML, ainsi que la chaîne de monitoring (section~\ref{sec:monitoring}) et Watchtower. Un unique fichier \code{docker-compose.yml}, versionné à la racine du dépôt, fait office de source de vérité unique pour les deux environnements : c'est exactement ce fichier que le pipeline copie et exécute sur l'instance de production (section~\ref{sec:cicd}), sans transformation intermédiaire. Les dépendances de démarrage sont explicitées via des conditions \code{depends_on} (par exemple l'API n'accepte des requêtes qu'une fois la base de données déclarée saine par son \emph{healthcheck}).

\subsection{Contrôler la mise en production de la solution}

Les quelques différences légitimes entre local et production (port du frontend, restriction des ports de monitoring à \code{127.0.0.1}, activation de Watchtower) sont pilotées par variables d'environnement à valeur par défaut adaptée au développement local, et par un mécanisme de \emph{profils} Docker Compose : les services de monitoring et Watchtower portent le profil \code{prod}, actif uniquement quand la variable \code{COMPOSE_PROFILES=prod} est présente dans le \code{.env} -- ce qui n'est le cas que sur l'instance EC2. Un second fichier, \code{docker-compose.override.yml}, est chargé automatiquement par Compose en local uniquement (il est ignoré dès qu'on invoque explicitement \code{-f docker-compose.yml}, ce que fait le pipeline) : il ajoute les instructions \code{build} et les montages du code source nécessaires au \emph{hot-reload} pendant le développement, sans jamais concerner la production, qui ne consomme que des images publiées sur Docker Hub. Le pipeline de déploiement inclut par ailleurs une étape de diagnostic automatique (relevé des journaux des conteneurs \code{postgres} et \code{backend\_api}) en cas d'échec du démarrage des services sur l'instance de production.
...
backend-api:
  image: ${BACKEND_API_IMAGE:-.../fanflight-backend-api}:${IMAGE_TAG:-latest}
  depends_on:
    db:
...
En local, \code{docker-compose.override.yml} (chargé automatiquement, listing~\ref{lst:override}) ajoute l'instruction \code{build} manquante pour reconstruire l'image depuis le code source plutôt que de tirer celle publiée sur Docker Hub :

\begin{lstlisting}[caption={\code{docker-compose.override.yml} -- extrait, build local uniquement}, label={lst:override}]
backend-api:
  build:
    context: ./API
...
Le second job du workflow copie par \code{scp} le \code{docker-compose.yml} du dépôt (et le dossier \code{monitoring/}) vers \code{/opt/fanflight} sur l'instance de production -- exactement le fichier décrit en~\ref{sec:cicd}, sans génération intermédiaire -- puis se connecte en SSH pour démarrer ou mettre à jour les services avec \code{docker compose -f docker-compose.yml up -d}. La variable \code{COMPOSE_PROFILES=prod}, ajoutée une seule fois dans le \code{.env} de l'instance, active le profil \code{prod} du fichier et démarre ainsi la chaîne de monitoring et le service \code{watchtower}, absents du profil par défaut utilisé en local. Watchtower assure ensuite la mise à jour continue des conteneurs en production, en interrogeant Docker Hub toutes les 60 secondes, sans nouvelle intervention du pipeline CI :
...
\label{sec:watchtower-fix}
La variable \code{DOCKER\_API\_VERSION} a été ajoutée en cours de projet à la suite d'un incident réel : le client Docker embarqué dans l'image Watchtower négociait par défaut une version d'API très ancienne (1.25), que le démon Docker de l'instance de production (API 1.55) rejette (\emph{minimum supported API version is 1.40}) -- Watchtower restait alors bloqué dans une boucle d'erreurs sans jamais parvenir à surveiller les conteneurs. Fixer explicitement une version récente et compatible (1.44) a résolu l'incident (figures~\ref{fig:alert-watchtower-log} et~\ref{fig:alert-watchtower-mail}).

\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{img/ec2-docker-ps.png}
\caption{\code{docker ps} sur l'instance de production : les 13 services du \code{docker-compose.yml} unique (application, ML, monitoring et Watchtower) actifs simultanément grâce au profil \code{prod}}
\label{fig:ec2-docker-ps}
\end{figure}

La figure~\ref{fig:ec2-docker-ps}, capturée directement sur l'instance EC2, confirme que les 13 services attendus tournent effectivement sous les noms de conteneur déclarés dans \code{docker-compose.yml} (\code{backend\_api}, \code{frontend}, \code{watchtower}, \code{prometheus}, \code{grafana}, etc.), la plupart marqués \emph{healthy} par leur \emph{healthcheck}, avec une durée de fonctionnement continue (\emph{Up 11 hours}) qui atteste qu'aucun de ces conteneurs -- Watchtower en particulier, cf. correctif de la section~\ref{sec:watchtower-fix} -- ne redémarre en boucle.
...
En complément du socle applicatif, Emy Regna a conçu et déployé, sur une branche dédiée (\code{monitoring-emy}) désormais fusionnée dans la branche principale, une chaîne d'observabilité complète de l'infrastructure FanFlight : supervision système, applicative et réseau, restituée dans trois tableaux de bord Grafana complémentaires (cf. section~\ref{sec:grafana-dashboards}). Ce travail démontre concrètement le volet « monitoring » de la compétence BC03 « Automatiser le déploiement de nouvelles versions de la solution et son monitoring ».
...
\begin{itemize}
  \item \textbf{\code{/metrics} natif de l'API} : \code{prometheus\_fastapi\_instrumentator} instrumente directement le code FastAPI pour exposer, par route, le nombre de requêtes, leur latence et la taille des réponses -- une métrique \emph{interne} à l'application ;
  \item \textbf{node-exporter} : métriques système de l'hôte (CPU, mémoire, disque, réseau) de l'instance EC2 de production ;
  \item \textbf{cAdvisor} : métriques par conteneur (CPU, mémoire, réseau, I/O disque), nécessitant un accès privilégié en lecture au socket Docker et au système de fichiers hôte ;
  ...
  \item \textbf{Blackbox Exporter} : \emph{health-checks} HTTP actifs sur le frontend et la route \code{/ready} de l'API -- une sonde \emph{externe} qui vérifie la disponibilité du service sans en connaître le détail interne, complémentaire du \code{/metrics} natif.
\end{itemize}
...
Un module Alertmanager, référencé par Prometheus via \code{rule\_files}, complète cette chaîne de collecte avec des règles d'alerte concrètes réparties en deux groupes : un groupe applicatif (indisponibilité de l'API, taux d'erreurs HTTP 5xx élevé, latence p95 dégradée, consommation mémoire excessive du service \code{backend-api}) et un groupe conteneurs, alimenté par cAdvisor (\code{ContainerDown} lorsqu'un conteneur n'est plus vu depuis plus d'une minute, \code{ContainerRestartingLoop} au-delà de trois redémarrages en 15 minutes). Alertmanager route chaque alerte vers une notification par e-mail (section~\ref{sec:alerting-proof}).
...
\subsection{Restitution : trois tableaux de bord Grafana provisionnés}
\label{sec:grafana-dashboards}

Grafana est connecté à Prometheus via une source de données provisionnée automatiquement au démarrage (fichier YAML, sans configuration manuelle dans l'interface), de même que trois tableaux de bord, chacun couvrant un niveau distinct de la stack : \emph{Fanflight -- Métriques applicatives} (niveau applicatif, conçu spécifiquement pour FanFlight), \emph{Docker monitoring} (niveau conteneur et hôte, également conçu pour ce projet) et \emph{Node Exporter Full} (niveau système, importé depuis la bibliothèque de tableaux de bord communautaires Grafana.com).

\subsubsection{Fanflight -- Métriques applicatives}

Ce tableau de bord restitue directement les métriques exposées par \code{prometheus\_fastapi\_instrumentator} : taux de requêtes global et par route, taux d'erreurs 5xx, latences p50/p95/p99 (globales et par \emph{endpoint}), taille moyenne des réponses, et consommation mémoire/CPU du processus API.

\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{img/grafana-app-metrics-1.png}
\caption{Fanflight -- Métriques applicatives : vue d'ensemble et trafic par \emph{endpoint}}
\label{fig:grafana-app-1}
\end{figure}

Sur la capture de la figure~\ref{fig:grafana-app-1}, prise sur l'environnement de recette, le trafic est volontairement synthétique (0,2 requête/s, générée par des appels de test) mais illustre le fonctionnement réel du tableau de bord : un taux d'erreurs 5xx à « No data » signifie qu'aucune erreur serveur n'a été observée sur la fenêtre considérée -- l'absence de donnée est ici une bonne nouvelle plutôt qu'un défaut de collecte -- et la latence p95 globale s'établit à 23,8~ms.

\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{img/grafana-app-metrics-2.png}
\caption{Fanflight -- Métriques applicatives : latences p50/p95/p99 globales et par \emph{endpoint}}
\label{fig:grafana-app-2}
\end{figure}

La figure~\ref{fig:grafana-app-2} détaille la distribution de latence : p50 à 13,2~ms, p95 à 27,5~ms (51,9~ms au pic), p99 à 50,3~ms (165~ms au pic) sur la fenêtre observée, avec un panneau dédié permettant d'identifier quel \emph{endpoint} (\code{/api/cities}, \code{/api/flights/\{match\_id\}}, \code{/api/matches}) contribue le plus à la queue de latence -- un niveau de granularité qui dépasse largement le simple \emph{health-check} de Blackbox Exporter.

\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{img/grafana-app-metrics-3.png}
\caption{Fanflight -- Métriques applicatives : taille des réponses et mémoire/CPU par processus}
\label{fig:grafana-app-3}
\end{figure}

Le dernier panneau (figure~\ref{fig:grafana-app-3}) va jusqu'à mesurer la latence des routes d'exploitation elles-mêmes (\code{/metrics} à 2,82~ms, \code{/ready} à 10,5~ms dans l'infobulle affichée), et suit la taille moyenne des réponses par route (jusqu'à 12~Kio) ainsi que la mémoire résidente et le CPU de chaque processus applicatif -- une instrumentation fine qui permettrait, par exemple, de détecter une fuite mémoire progressive avant qu'elle ne cause un incident.

\subsubsection{Node Exporter Full}

Plutôt que de redévelopper un tableau de bord système from scratch, l'équipe a importé \emph{Node Exporter Full}, le tableau de bord communautaire de référence pour node-exporter (plusieurs dizaines de panneaux couvrant CPU, mémoire, disque, réseau et processus) -- un choix pragmatique qui illustre la compétence de veille technologique déjà mobilisée par ailleurs dans le projet.

\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{img/grafana-node-exporter-1.png}
\caption{Node Exporter Full : vue de synthèse CPU/mémoire/disque de l'instance EC2 de production}
\label{fig:grafana-node-1}
\end{figure}

La capture de la figure~\ref{fig:grafana-node-1}, prise directement sur l'instance EC2 de production, montre des chiffres réels d'exploitation : une disponibilité de 17,9 semaines sans interruption, 2 cœurs CPU, un usage disque racine à 86,5~\% (29~Gio au total) et une charge système (\emph{load average}) rapportée à 176,5~\% -- une valeur à surveiller qui dépasse la capacité nominale de la machine et qui constitue un signal concret pour les préconisations de dimensionnement de la section~\ref{sec:preconisations}.

\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{img/grafana-node-exporter-2.png}
\caption{Node Exporter Full : liste des sections disponibles (mémoire, réseau, stockage, processus)}
\label{fig:grafana-node-2}
\end{figure}

La figure~\ref{fig:grafana-node-2} donne un aperçu de l'exhaustivité du tableau de bord importé : au-delà de la vue de synthèse, il expose des sections dépliables dédiées à la mémoire détaillée, à la synchronisation temporelle, aux processus système, au stockage (disque et système de fichiers) et au réseau (trafic, sockets, \emph{netstat}) -- une profondeur d'analyse qu'il aurait été disproportionné de reconstruire manuellement pour ce projet.

\subsubsection{Docker monitoring}

Un troisième tableau de bord, \emph{Docker monitoring}, comble l'angle mort laissé par les deux précédents : la consommation par conteneur individuel (mémoire, CPU, réseau, I/O disque, nombre de redémarrages) plutôt que par processus applicatif ou par hôte global. Il combine des panneaux par conteneur (\emph{Memory Usage -- Containers}, \emph{CPU Usage -- Containers}, \emph{Network Rx/Tx -- Containers}, \emph{Disk Read/Write I/O -- Containers}, \emph{Container Restarts}) avec des panneaux de synthèse au niveau de l'hôte (\emph{Total CPU/Memory Usage -- EC2}, \emph{EC2 Load Average}, \emph{Disk Usage -- EC2}), permettant de relier directement une dérive de ressource observée au niveau système à un conteneur précis.

\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{img/docker-monitoring-1.png}
\caption{Docker monitoring : vue de synthèse (13 conteneurs actifs) et détail CPU/mémoire par conteneur}
\label{fig:docker-mon-1}
\end{figure}

La capture de la figure~\ref{fig:docker-mon-1}, prise en production, dénombre 13 conteneurs actifs (l'ensemble de la stack applicative et de monitoring) et détaille la mémoire consommée par chacun : \code{backend\_api} domine largement (343~Mio), loin devant \code{frontend} (46,0~Mio), \code{postgres\_exporter} (15,6~Mio) et \code{node\_exporter} (18,7~Mio) -- les exporters et Watchtower ne pèsent chacun que quelques Mio.

\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{img/docker-monitoring-2.png}
\caption{Docker monitoring : I/O disque par conteneur, redémarrages, erreurs réseau et charge de l'hôte EC2}
\label{fig:docker-mon-2}
\end{figure}

La figure~\ref{fig:docker-mon-2} confirme, sur la même fenêtre d'observation, zéro redémarrage de conteneur, zéro erreur ou paquet réseau perdu, mais un disque racine déjà occupé à 74,4~\% -- cohérent avec le taux d'usage disque relevé sur le tableau de bord \emph{Node Exporter Full} (figure~\ref{fig:grafana-node-1}) et avec la préconisation de dimensionnement associée (section~\ref{sec:preconisations}).

\subsection{Intégration au déploiement}

La chaîne d'observabilité est définie dans le \code{docker-compose.yml} unique du projet (section~\ref{sec:cicd}), derrière le profil \code{prod} activé en production et activable à la demande en local (\code{docker compose --profile prod up -d}) pour reproduire la même configuration avant tout déploiement réel. Son déploiement en production est intégré au workflow GitHub Actions : les fichiers de configuration (Prometheus, Alertmanager, provisioning Grafana et les trois définitions de tableaux de bord) sont copiés vers \code{/opt/fanflight/monitoring/} sur l'instance EC2, et les identifiants d'administration Grafana sont paramétrables par variable d'environnement plutôt que codés en dur.

\subsection{Alertes par e-mail : preuve de fonctionnement en conditions réelles}
\label{sec:alerting-proof}

Alertmanager relaie chaque alerte par e-mail via un bac à sable SMTP (Mailtrap), avec un regroupement des alertes similaires (\code{group\_wait} de 30 secondes, \code{group\_interval} de 5 minutes) et une relance si l'incident persiste (\code{repeat\_interval} de 4 heures) ; chaque notification de résolution est également envoyée (\code{send\_resolved: true}). Trois épisodes réels, survenus en cours de projet, illustrent que cette chaîne fonctionne effectivement de bout en bout plutôt que sur le papier.

\begin{figure}[h]
\centering
\includegraphics[width=0.8\textwidth]{img/alert-api-down-mail.png}
\caption{Boîte de réception Mailtrap : alternance \code{[FIRING:1]}/\code{[RESOLVED]} pour \code{FanflightAPIDown}}
\label{fig:alert-api-down}
\end{figure}

La figure~\ref{fig:alert-api-down} montre l'alerte applicative \code{FanflightAPIDown} déclenchée puis résolue à plusieurs reprises sur deux jours -- chaque cycle correspond à une indisponibilité réelle de l'API détectée par Prometheus, suivie de sa récupération.

\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{img/alert-watchtower-log.png}
\caption{Journal du conteneur Watchtower au moment de l'incident : API Docker trop ancienne (cf. section~\ref{sec:watchtower-fix})}
\label{fig:alert-watchtower-log}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.6\textwidth]{img/alert-watchtower-mail.png}
\caption{Alerte \code{ContainerDown} correspondante, reçue par e-mail pour le conteneur \code{fanflight\_watchtower}}
\label{fig:alert-watchtower-mail}
\end{figure}

Les figures~\ref{fig:alert-watchtower-log} et~\ref{fig:alert-watchtower-mail} documentent un même incident réel sous deux angles : le rejet des requêtes de Watchtower par le démon Docker (version d'API cliente trop ancienne, cf. section~\ref{sec:cicd}) a fini par rendre le conteneur invisible pour cAdvisor, déclenchant l'alerte \code{ContainerDown} correspondante -- une preuve concrète que la règle conçue pour détecter un conteneur arrêté ou planté fonctionne également sur ce type de panne applicative.

\begin{figure}[h]
\centering
\includegraphics[width=0.6\textwidth]{img/alert-frontend-down-mail.png}
\caption{\code{[3] Firing} : trois alertes \code{ContainerDown} groupées pour le service \code{frontend}}
\label{fig:alert-frontend-down}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.6\textwidth]{img/alert-frontend-resolved-mail.png}
\caption{\code{[4] Resolved} : résolution du même incident sur le conteneur \code{frontend}}
\label{fig:alert-frontend-resolved}
\end{figure}

Les figures~\ref{fig:alert-frontend-down} et~\ref{fig:alert-frontend-resolved} illustrent un second incident \code{ContainerDown}, cette fois sur le conteneur \code{frontend}, avec le comportement de regroupement (\emph{group\_by: [alertname, service]}) : plusieurs occurrences de la même alerte sont fusionnées dans un seul e-mail plutôt que d'en envoyer une par occurrence, puis un e-mail de résolution distinct est envoyé au retour à la normale.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{img/mlflow-experiments.png}
\caption{Interface MLflow -- expérience \code{flight\_delay\_prediction} : le run parent \code{FlightDelay\_Comparison} et ses deux runs enfants \code{XGBoost}/\code{NeuralNetwork}}
\label{fig:mlflow-experiments}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{img/mlflow-run-detail.png}
\caption{Détail du run \code{FlightDelay\_Comparison} : paramètres et métriques de comparaison}
\label{fig:mlflow-run-detail}
\end{figure}

Les figures~\ref{fig:mlflow-experiments} et~\ref{fig:mlflow-run-detail} montrent une comparaison réelle, journalisée automatiquement par \code{train.py} sur 628\,704 lignes d'entraînement (20 features, 20~\% de jeu de test, taux de retard observé de 27,8~\%) : l'AUC final de XGBoost (0,728) dépasse celui du réseau de neurones (0,711), ce qui justifie la promotion de XGBoost comme \code{best\_model} / \code{production\_model} -- exactement le modèle \code{flight\_delay\_xgboost} chargé au démarrage de l'API ci-dessous.
...
\begin{itemize}
  ...
  \item étendre les règles d'alerte Alertmanager déjà en place (indisponibilité API, erreurs 5xx, latence, mémoire) à la charge système de l'instance EC2 : le tableau de bord \emph{Node Exporter Full} y a fait apparaître une charge (\emph{load average}) rapportée à 176,5~\%, au-delà de la capacité nominale de la machine à 2 cœurs (cf. section~\ref{sec:grafana-dashboards}), ce qui justifie une alerte dédiée plutôt qu'une simple observation manuelle ;
